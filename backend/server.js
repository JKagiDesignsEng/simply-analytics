...
app.post('/api/track', trackingLimiter, async (req, res) => {
  try {
    ...
    if (!finalWebsiteId) {
      return res
        .status(400)
        .json({ error: 'Website ID or domain is required' });
    }
    ...
    try {
      ...
      if (avgLoadTime > 3000) {
        insights.push({
          ...
          message: `Average page load time is ${Math.round(
            avgLoadTime / 1000
          )}s. Consider optimizing images, minifying assets, or using a CDN.`,
        ...
        insights.push({
          ...
          message: `Your pages load in ${Math.round(
            avgLoadTime / 1000
          )}s on average. Keep up the good work!`,
          metric: Math.round(avgLoadTime),
        });
      }
      ...
      const singlePageSessions = parseInt(
        bounceResult.rows[0]?.single_page_sessions || 0
      );
      const bounceRate =
      ...
      if (bounceRate > 70) {
        insights.push({
          ...
          message: `${Math.round(
            bounceRate
          )}% of visitors leave after viewing just one page. Improve content relevance and internal linking.`,
        ...
        insights.push({
          ...
          message: `Only ${Math.round(
            bounceRate
          )}% bounce rate. Your visitors are exploring multiple pages!`,
          metric: Math.round(bounceRate),
        });
      }
      ...
      const totalViews = deviceResult.rows.reduce(
        (sum, row) => sum + parseInt(row.count),
        0
      );
      const mobileViews =
        deviceResult.rows.find((r) => r.device_type === 'mobile')?.count || 0;
      const mobilePercentage =
        totalViews > 0 ? (mobileViews / totalViews) * 100 : 0;
      ...
      if (mobilePercentage > 60) {
        insights.push({
          ...
          message: `${Math.round(
            mobilePercentage
          )}% of your traffic is mobile. Ensure your site is fully responsive and mobile-optimized.`,
      ...
      if (
        trendingResult.rows.length > 0 &&
        trendingResult.rows[0].growth_rate > 50
      ) {
        insights.push({
          type: 'success',
          category: 'trending',
          title: 'Trending Content Detected',
          message: `${trendingResult.rows[0].path} is gaining ${Math.round(
            trendingResult.rows[0].growth_rate
          )}% more views. Consider promoting similar content!`,
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        ...
        highDpiPercentage:
          highDpi.rows[0].total_views > 0
            ? (highDpi.rows[0].high_dpi_views / highDpi.rows[0].total_views) *
              100
            : 0,
      });
    } catch (error) {
